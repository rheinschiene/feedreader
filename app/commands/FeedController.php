		...
		if(!empty($xpathContent)) {
		    ...
		    if($nodes->length < 2) {
		        ...
		        foreach($nodes as $node) {
					...
					$nodeValue = str_replace("Ä","A_e",$nodeValue);
					$nodeValue = str_replace("ß","_ss_",$nodeValue);
		            $nodeValue = filter_var($nodeValue, FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_LOW | FILTER_FLAG_STRIP_HIGH);
					...
					$nodeValue = str_replace("A_e","Ä",$nodeValue);
					$nodeValue = str_replace("_ss_","ß",$nodeValue);
